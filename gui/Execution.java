...
public class Execution {
	...
	public boolean createNetwork() {
		...
		// System.out.print("\n Test Network with genome -> " +
		// EnvConstant.NAME_OF_GENOME_FOR_EXECUTION);
		...
		if (ret) {
			...
			try {
				...
				System.out.print("\n execution: : error during read "
						+ EnvConstant.NAME_OF_GENOME_FOR_EXECUTION + " file "
						+ e);
				return false;
			}
			...
			System.out.print("\n generation: error in open genome "
					+ EnvConstant.NAME_OF_GENOME_FOR_EXECUTION);
			return false;
	...
	public void createNetwork(Genome _genome) {
		...
		// generate genome from file
		Xgenome = new Genome(_genome.genome_id, _genome.getTraits(),
				_genome.getNodes(), _genome.getGenes());
	...
	public void executeForClass(JTextArea _textArea) {
		...
		try {
			...
			EnvConstant.NR_UNIT_OUTPUT = Integer
					.parseInt(ObjRet_tgt.toString());
		...
		_textArea.append("\n\nPOP.    FILE : "
				+ EnvConstant.CURRENT_POPULATION_VIEW);
		_textArea.append("\nGENOME  FILE : "
				+ EnvConstant.NAME_OF_GENOME_FOR_EXECUTION);
		...
		try {
			...
			for (count = 0; count < EnvConstant.NUMBER_OF_SAMPLES; count++) {
				...
				for (int j = 0; j < nr_output; j++) {
					double v1 = ((NNode) Xnet.getOutputs().elementAt(j))
							.getActivation();
					out[count][j] = v1;
				}
			...
			System.out
					.print("\n Error(2) generic in execution.executeForClass: code "
							+ e2);
			return;
		}
	...
	public void executeForFile(JTextArea _textArea) {
		...
		// System.out.print("\n open .." +
		// EnvRoutine.getJneatFile(EnvConstant.DATA_INP));
		boolean ret = xFile.IOseqOpenR();
		...
		// System.out.print("\n open .." +
		// EnvRoutine.getJneatFile(EnvConstant.DATA_OUT));
		...
		try {
			...
			System.out
					.print("\n Error(1) generic in execution.executeForFile: code "
							+ e2);
			return;
		}
		...
		while ((xline != "EOF")) {
			...
			if (sz != nr_input) {
				...
				System.out.print(" different from declared " + nr_input
						+ " unit");
				System.out.print("\n correct and re-run;\n\t  Bye");
				return;
			}
		...
		try {
			...
			System.out
					.print("\n Error(3) generic in execution.executeForFile: code "
							+ e2);
			return;
		}
