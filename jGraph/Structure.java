...
public class Structure {
			...
			try {
				...
				while (!done) {
					...
					if (curword.equalsIgnoreCase("genomeend")) {
						...
						// Get recur
						curword = st.nextToken();
						boolean recur = Integer.parseInt(curword) == 1 ? true
								: false;
						...
						// Get enable
						curword = st.nextToken();
						boolean enable = Integer.parseInt(curword) == 1 ? true
								: false;
						...
						if (enable) {
							...
							newedge = new Edge(iVertex, oVertex, recur,
									CodeConstant.LINE_SE, weight);
		...
		while (itr_edge.hasNext()) {
			...
			if (diff_level == 0) {
				...
				if (_inode.name == _onode.name) {
					...
					if (!found) {
						System.out
								.print("\n *ALERT* : non trovato laver per nodo :"
										+ _inode.name);
					}
	...
	public void generate_planeXY() {
		...
		while (itr_plane.hasNext()) {
			...
			if (_plane.vEdge.size() > 0) {
				...
				// System.out.print("\n Il piano di livello "+_plane.level
				// 
					...
					if (diff_level > 1) {
						...
						if (_inode.gx < _onode.gx) {
							...
							for (int colonna = (from_node.gx + 1); colonna < to_node.gx; colonna++) {
								...
								// System.out.print("\n added node virtual Horizontal["
								// + virtP.name + "] con level=" + virtP.level);
	 ...
	 * 
	 * = al numero REALE di livelli che quindi si estenderanno tra 0 e
	 * (depth_y-1)
	 */
	...
	public void compute_depth_y() {
		 ...
		 * "\n ******************************************************"); //
		 * view();
		 * System.out.print("\n ******* compute.depth.y : max altitude_b =  "
		 * +max_b);
		...
		while (changed) {
			...
			while (itr_Vertex.hasNext()) {
				...
				// System.out.print(" che ha level = "+
				// _Vertex.level+" altitude = "+_Vertex.altitude);
				...
				if ((_Vertex.level == -1) && (_Vertex.altitude == -1)) {
					...
					while (itr_Vertey.hasNext()) {
						...
						// _Vertey.level+" altitude = "+_Vertey.altitude);
					...
					// se non ci sono stati cambiamenti riprovo in senso opposto
						...
						while (itr_Vertey.hasNext()) {
							...
							if ((_Vertey.altitude >= 0)
									&& (_Vertey.altitude < max)) {
								found = true;
								if (_Vertey.altitude < xmin) {
									xmin = _Vertey.altitude;
									_Vz = _Vertey;
		...
		while (itr_Vertex.hasNext()) {
			...
			if ((_Vertex.level == -1) && (_Vertex.altitude == -1)) {
				...
				while (itr_Edge.hasNext()) {
					_Edge = ((Edge) itr_Edge.next());
					if ((_Edge.in_node == _Vertex)
							|| (_Edge.out_node == _Vertex)) {
	...
	public void compute_Coordinate(int xm, int ym) {
		try {
			...
			System.out.print("\n *alert* error in computation of coordinate"
					+ e);
		}
