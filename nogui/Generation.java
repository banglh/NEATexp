...
public class Generation extends JPanel implements ActionListener, ItemListener {
	...
	public Generation(JFrame _f) {
		...
		paneScroll2.setBorder(BorderFactory.createCompoundBorder(
		...
		try {
			...
			if (!EnvConstant.REPORT_SPECIES_TESTA.equalsIgnoreCase("")) {
				...
				doc2.insertString(doc2.getLength(), "\n\n GENERATION : "
						+ generation, textPane2.getStyle(My_styles[2]));
				doc2.insertString(doc2.getLength(),
						EnvConstant.REPORT_SPECIES_TESTA,
						textPane2.getStyle(My_styles[1]));
				doc2.insertString(doc2.getLength(),
						EnvConstant.REPORT_SPECIES_CORPO,
						textPane2.getStyle(My_styles[3]));
				doc2.insertString(doc2.getLength(),
				...
				if (!(EnvConstant.FIRST_ORGANISM_WINNER == null)) {
					...
					drawGraph((Organism) EnvConstant.FIRST_ORGANISM_WINNER,
							riga1, mappa_graph);
			...
			drawCurve(riga1, pop.getHighest_fitness(), generation, pop
					.getSpecies().size(), _neat.p_pop_size);
